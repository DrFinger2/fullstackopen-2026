...
function formatUrl(urlInput) {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (
      c === 58 &&
      i + 2 < end &&
      urlInput.charCodeAt(i + 1) === 47 &&
      urlInput.charCodeAt(i + 2) === 47
    ) {
      // ://
      sepIdx = i
      break
  ...
  if (sepIdx !== -1) {
    ...
    if (c0 === 109 && remLen >= 7 && urlInput.charCodeAt(start + 6) === 58) {
      // mailto:
      if (urlInput.slice(start, start + 7).toLowerCase() === 'mailto:')
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
    } else if (
      c0 === 106 &&
      remLen >= 11 &&
      urlInput.charCodeAt(start + 10) === 58
    ) {
      // javascript:
      if (urlInput.slice(start, start + 11).toLowerCase() === 'javascript:')
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
    } else if (
      c0 === 116 &&
      remLen >= 4 &&
      urlInput.charCodeAt(start + 3) === 58
    ) {
      // tel:
      if (urlInput.slice(start, start + 4).toLowerCase() === 'tel:')
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
    } else if (
      c0 === 100 &&
      remLen >= 5 &&
      urlInput.charCodeAt(start + 4) === 58
    ) {
      // data:
      if (urlInput.slice(start, start + 5).toLowerCase() === 'data:')
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
    } else if (
      c0 === 117 &&
      remLen >= 4 &&
      ...
      // urn:
      if (urlInput.slice(start, start + 4).toLowerCase() === 'urn:')
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
    } else if (
      c0 === 97 &&
      remLen >= 6 &&
      urlInput.charCodeAt(start + 5) === 58
    ) {
      // about:
      if (urlInput.slice(start, start + 6).toLowerCase() === 'about:')
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
    }
  }
  ...
  for (let i = hostStart; i <= hostEnd; i++) {
    if (i === hostEnd || urlInput.charCodeAt(i) === 46) {
      // '.' or end of host
      ...
      if (
        urlInput.charCodeAt(segStart) === 45 ||
        urlInput.charCodeAt(i - 1) === 45
      ) {
        return { isValid: false, formattedUrl: urlInput.slice(start, end) }
      }
  ...
  if (portStr) {
    const isDefault =
      (scheme === SCHEME_HTTP && portNum === 80) ||
      (scheme === SCHEME_HTTPS && portNum === 443)
    if (!isDefault) {
