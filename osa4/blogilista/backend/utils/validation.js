...
const allowedUsernameSymbol = Object.freeze([
    '_', '-', '.'
])
...
function validateUsername(username) {
    ...
    if (username.length < USERNAME_MIN) {
        return { ok: false, error: `Username needs to be at least ${USERNAME_MIN} characters long` }
    }
    if (username.length > USERNAME_MAX) {
        return { ok: false, error: `Username needs to be fewer than ${USERNAME_MAX} characters long` }
    }

    for (const char of username) {
        if (!isAlphabetic(char) && !isDigit(char) && !isUsernameSymbol(char)) {
            return { ok: false, error: 'Username can only contain letters, numbers, underscores, hyphens, and periods' }
        }
    }
...
function validatePassword(password) {
    ...
    if (password.length < PASSWORD_MIN) {
        return { ok: false, error: `Password needs to be at least ${PASSWORD_MIN} characters long` }
    }
